...
const Publish: React.FC<PublishProps> = ({ id, data }) => {
  ...
  const handleLikePost = async () => {
    ...
    if (likeResponse.statusCode === 200) {
      ...
      setLockButton(true);
      enqueueSnackbar("Publicação curtida com sucesso!", {
        variant: "success",
      });
    } else {
  ...
  return (
    <div className="publish-container">
      <div className="publish-content publish-content-data">
        <h1 className="publish-title">{data.title}</h1>
          ...
          <div className="publish-details-flex">
            <div className="flex items-center">
              {lockButton ? (
                <AiFillHeart
                  size={16}
                  className="mr-1 text-red cursor-pointer"
                />
              ) : (
                <AiOutlineHeart
                  size={16}
                  className="mr-1 text-dark-blue cursor-pointer"
            ...
            <div className="post-card-content">
              <div className="flex items-center">
                <AiFillCalendar
                  size={14}
                  className="inline-block mr-1 text-dark-blue"
                />
              ...
              <div className="flex items-center">
                <AiFillClockCircle
                  size={14}
                  className="mr-1 text-dark-blue hidden 2xs:flex"
                />
                <span className="text-sm text-gray-500">
                  {data.timeToRead} min.
                </span>
        ...
        <Image
          ...
          height={1000}
        />
        <div className="ql-tags-container text-black text-lg mt-4">
          {parse(data.content)}
        </div>
        ...
        <div className="publish-details-flex">
          <div className="flex items-center">
            {lockButton ? (
              <AiFillHeart size={16} className="mr-1 text-red cursor-pointer" />
            ) : (
              <AiOutlineHeart
                size={16}
                className="mr-1 text-dark-blue cursor-pointer"
