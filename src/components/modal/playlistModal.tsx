...
const PlaylistModal: React.FC<PlaylistModalProps> = ({ isOpen, onClose, musicContent }) => {
  ...
  return (
    <div className={`${isOpen ? 'visible' : ' invisible opacity-0'} flex justify-center items-center fixed top-0 left-0 w-full h-full bg-black-alpha z-50 transition-all`}>
      <div ref={ref} className="relative overflow-hidden rounded-lg lg:w-96 lg:h-96 md:w-96 md:h-96 opacity-100 w-4/5 h-2/5">
        {(musicContent.image &&
          <Image
            className="w-full h-full scale-110 object-cover rounded-lg"
            src={musicContent.image}
            alt="music-image"
          />
        )}
